...
final case class IndexedContsT[W[_], M[_], R, O, A](_run: W[A => M[O]] => M[R]) {
  ...
  def flatten[E, B](implicit ev: A =:= IndexedContsT[W, M, O, E, B], W: CoflatMap[W]): IndexedContsT[W, M, R, E, B] = flatMap(ev)

  def flatMap[E, B](f: A => IndexedContsT[W, M, O, E, B])(implicit W: CoflatMap[W]): IndexedContsT[W, M, R, E, B] =
    IndexedContsT { wbme =>
      run(W.coflatMap(wbme) { wk => { a =>
        f(a).run(wk)
      } })
    }
  ...
  def leftMap[E](f: R => E)(implicit M: Functor[M]): IndexedContsT[W, M, E, O, A] =
    IndexedContsT { wamo =>
      M.map(run(wamo))(f)
    }
  ...
  def plus(that: IndexedContsT[W, M, R, O, A])(implicit M: SemigroupK[M]): IndexedContsT[W, M, R, O, A] =
    IndexedContsT { wamo =>
      M.combineK(this(wamo), that(wamo))
    }
}
...
trait IndexedContsTFunctions {
  def pure[W[_], M[_], R, A](a: => A)(implicit W: Comonad[W]): ContsT[W, M, R, A] =
    ContsT { k => W.extract(k)(a) }

  def empty[W[_], M[_], R, A](implicit M: MonoidK[M]): ContsT[W, M, R, A] =
    ContsT { k => M.empty }

  def liftM[W[_], M[_], R, A](a: => M[A])(implicit W: Comonad[W], M: FlatMap[M]): ContsT[W, M, R, A] =
    ContsT { k => M.flatMap(a)(W.extract(k)) }
  ...
  def shift[W[_], M[_], I, R, J, O, A](f: (A => IndexedContsT[W, M, I, I, O]) => IndexedContsT[W, M, R, J, J])(implicit W: Comonad[W], WA: Applicative[W], M: Monad[M]): IndexedContsT[W, M, R, O, A] =
    IndexedContsT { k0 =>
      (f { a =>
        IndexedContsT { k1 =>
          M.flatMap(W.extract(k0)(a))(W.extract(k1))
        }
      }).run_
    }
...
sealed abstract class IndexedContsTInstances1 extends IndexedContsTInstances2 {
  // cats requires FlatMap to implement tailRecM
  /*
  implicit def ContsTBind[W[_], M[_], R](implicit W0: CoflatMap[W]): FlatMap[ContsT[W, M, R, *]] =
...
private sealed trait IndexedContsTFunctorLeft[W[_], M[_], O, A0] extends Functor[IndexedContsT[W, M, *, O, A0]] {
  ...
  override def map[A, B](fa: IndexedContsT[W, M, A, O, A0])(f: A => B): IndexedContsT[W, M, B, O, A0] = fa.leftMap(f)
}
...
private sealed trait IndexedContsTContravariant[W[_], M[_], R, A0] extends Contravariant[IndexedContsT[W, M, R, *, A0]] {
  ...
  override def contramap[A, B](fa: IndexedContsT[W, M, R, A, A0])(f: B => A): IndexedContsT[W, M, R, B, A0] = fa.contramap(f)
}
...
private sealed trait IndexedContsTBifunctor[W[_], M[_], O] extends Bifunctor[IndexedContsT[W, M, *, O, *]] {
  ...
  override def bimap[A, B, C, D](fab: IndexedContsT[W, M, A, O, B])(f: A => C, g: B => D): IndexedContsT[W, M, C, O, D] = fab.bimap(f, g)

  override def leftFunctor[X]: Functor[IndexedContsT[W, M, *, O, X]] = IndexedContsT.IndexedContsTFunctorLeft

  override def leftMap[A, B, C](fa: IndexedContsT[W, M, A, O, B])(f: A => C): IndexedContsT[W, M, C, O, B] = fa.leftMap(f)
...
private sealed trait ContsTMonad[W[_], M[_], R] extends Monad[ContsT[W, M, R, *]] with ContsTFlatMap[W, M, R] with StackSafeMonad[ContsT[W, M, R, *]] {
  ...
  override def pure[A](x: A): ContsT[W, M, R, A] = IndexedContsT.pure(x)
}
...
private sealed trait ContsTAlternative[W[_], M[_], R] extends Alternative[ContsT[W, M, R, *]] with ContsTMonad[W, M, R] {
  ...
  override def combineK[A](x: ContsT[W, M, R, A], y: ContsT[W, M, R, A]): ContsT[W, M, R, A] = x.plus(y)
}
